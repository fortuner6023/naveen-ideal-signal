...
/*-----------------------------------------------
  # Variables
  ---------------------------------------------*/
$MailchimpAPI 	= '319190644872dce6ceb44a5f3dea07f0-us5'; //<- Your Maichimp API Key
$MailchimpLID 	= 'c103bc3c6a'; //<- Your Maichimp List IDkey
...
if ( ! empty( $email ) ) {
	...
	if ( $email == $clean_email && filter_var( $email, FILTER_VALIDATE_EMAIL ) ){
		...
		$msg 	.= '<strong>Congrats!</strong> You will receive the news about our upcoming updates!';
		$msg 	.= '<br>';

	} else {

		$error 	 = true;
		$msg 	.= '<strong>Error:</strong> Invalid email address, confirm your email address and try again.';
		$msg 	.= '<br>';
